package com.example.usuario.herramientadelmayordomo.Almacenamiento;
...
public class MySharedPreferences {
    ...
    private static final String KEY_DEFAULT_MAIL_LOCKED = "default_mail_locked";
    ...
    public static boolean getDefaultMailLocked(Context ctx){
        SharedPreferences preferences = ctx.getSharedPreferences(NAME_ARCHIVO,MODE_APPEND);
        return preferences.getBoolean(KEY_DEFAULT_MAIL_LOCKED,false);
    }

    public static void storeDefaultMail(Context ctx,String mail,boolean locked){
        ...
        editor.putBoolean(KEY_DEFAULT_MAIL_LOCKED,locked);
        editor.apply();
    ...
    public static int getNotificationId(Context ctx){
        SharedPreferences preferences = ctx.getSharedPreferences(MySharedPreferences.NAME_ARCHIVO,MODE_APPEND);
        if(preferences.getInt(MySharedPreferences.KEY_NOTIFICATION_ID,0)==50){
            storeNotificationId(ctx,0);
            return 50;
        }
        int id = preferences.getInt(MySharedPreferences.KEY_NOTIFICATION_ID,0);
        storeNotificationId(ctx,id+1);
        return id;
    }

    private static void storeNotificationId(Context ctx, int id){
